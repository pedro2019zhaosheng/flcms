 ...
 * @blog https://cnblogs.com/hellow-world
 */
class JclqBase extends BaseModel
{
    /**
     * 公共筛选方法
     *
    ...
    public function deleteAll($where)
    {
        ...
        $result = JclqOpen::where('match_num', 'in', $matchNum)->where('status', 0)->find();
        if (!empty($result)) {
        ...
        try {
            Db::startTrans();
            JclqOpen::where('match_num', 'in', $matchNum)->delete();
            JclqMatch::where('match_num', 'in', $matchNum)->delete();
    ...
    {
        return self::alias('a')
            ->leftJoin('jclq_match b', 'a.match_num=b.match_num')
            ->where($where)
            ->field($data)
            ->group('a.match_num')
            ->order($order)
 ...
 */
     ...
     * @api *
     */
    public static function getRfs($matchNum)
    {
