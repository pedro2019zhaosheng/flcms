 ...
 * @blog https://cnblogs.com/hellow-world
 */
class JcdcMatch extends BaseModel
{
    use Data;
    ...
    public function getDetail($matchId)
    {
        ...
        $data['sp_sxp'] = Helper::jsonDecode($data['sp_sxp']);
        ...
        $data['sp_sxp_var'] = $this->odds($data['sp_sxp_var']);
    ...
    public function handingBetas($bet_content)
    {
        ...
        foreach ($betContentBody as &$v) {//循环跟单订单的投注项
            $jcdcMatchData = self::getDetail($v['mnum']);//获取当前赛事的奖金指数
            foreach($v['muti'] as &$value) {
                ...
                switch ($ptype) {
                    ...
                    case "sp_rqspf"://让球胜平负
                        foreach ($bet as $i => $va) {
                            $str = $str . $jcdcMatchData['sp_spf'][$va] . '|';
                            $gameIndex[$v['mnum']][$value['ptype']][$va] = $jcdcMatchData['sp_spf'][$va];
                        }
                    ...
                    case "sp_bf"://比分
                        foreach ($bet as $va) {
                            $str = $str . $jcdcMatchData['sp_bf']['dat'][$va] . '|';
                            $gameIndex[$v['mnum']][$value['ptype']][$va] = $jcdcMatchData['sp_bf']['dat'][$va];
                        }
                        $value['i'] = trim($str, '|');//重新赋值奖金指数
                        //$gameIndex[$v['mnum']][$value['ptype']] = trim($str, '|');
                        break;
                    case "sp_jqs"://进球数
                        foreach ($bet as $va) {
                            $str = $str . $jcdcMatchData['sp_jqs']['dat'][$va] . '|';
                            $gameIndex[$v['mnum']][$value['ptype']][$va] = $jcdcMatchData['sp_jqs']['dat'][$va];
                        }
                        $value['i'] = trim($str, '|');//重新赋值奖金指数
                        //$gameIndex[$v['mnum']][$value['ptype']] = trim($str, '|');
                        break;
                    case "sp_bqc"://半全场
                        ...
                        foreach ($bet as $va) {
                            $str = $str . $jcdcMatchData['sp_bqc']['dat'][$va] . '|';
                            $gameIndex[$v['mnum']][$value['ptype']][$va] = $jcdcMatchData['sp_bqc']['dat'][$va];
                        }
                        $value['i'] = trim($str, '|');//重新赋值奖金指数
                        //$gameIndex[$v['mnum']][$value['ptype']] = trim($str, '|');
                        break;
