 ...
 * @blog https://cnblogs.com/hellow-world
 */
class JcdcOpen extends BaseModel
{
    ...
    public function handOpen($matchNum)
    {
        ...
        //+------------------------------------------------------------------------------
        $dealData = $this->matchResult2JcResult(Config::BJ_CODE, $openDetailData);
        // 获取彩种ID
        $lotteryId = Lottery::getIdByCode(Config::BJ_CODE);
        // 该赛事所有订单数据
        ...
        foreach ($data as $datum) {
            try {
                ...
                if (!$unOpenDrawCount && !$unDrawCount) {
                    ...
                    if ($isYh == 0) {
                        $bonus = round($double * $oneZhuAmount * $product * 0.65, 2); // 倍数*每注金额*赔率*0.6
                    } else {
                        // 获取该注的优化倍数
                        $yhDouble = OrderContent::getValByWhere(['id' => $datum['order_content_id']], 'beishu');
                        $bonus = round($yhDouble * $oneZhuAmount * $product * 0.65, 2); // 优化倍数*每注金额*赔率*0.6
                    }
                ...
                $isComplete = false;
                // 记录系统日志
                Helper::log('系统', '北京单场手动开奖', "注单{$datum['order_no']}开奖失败!", $e->getMessage(), 0);
            }
        }

        // 设置该赛事已停售
        JcdcBase::where('match_num', $matchNum)->setField('sale_status', 0);
        // 判断是否完成开奖
        if ($isComplete) {
            return true;
        }
     ...
     * @api *
     */
    public function dcAutoDraw($matchNum)
    {
        // 获取赛事结果
        $openDetail = $this->getMatchDetail($matchNum);
        if (
        ...
        foreach ($data as $datum) {
            try {
                ...
                if (!$unOpenDrawCount && !$unDrawCount) {
                    ...
                    $double = (int)$datum['beishu']; // 倍数
                    if ($isYh == 0) {
                        $bonus = round($double * $oneZhuAmount * $product * 0.65, 2); // 倍数*每注金额*赔率
                    } else {
                        // 获取该注的优化倍数
                        $yhDouble = OrderContent::getValByWhere(['id' => $datum['order_content_id']], 'beishu');
                        $bonus = round($yhDouble * $oneZhuAmount * $product * 0.65, 2); // 优化倍数*每注金额*赔率
                    }
                    // 设置订单内容中, 该注订单为已中奖, 并写入奖金
                    OrderContent::where('id', $datum['order_content_id'])->setField(['status' => 1, 'bonus' => $bonus]);
                    // 查询订单内容中是否存在未开奖的数据
                ...
                Db::rollback();
                // 写入日志
                Helper::log('系统', '北京单场自动开奖', "注单{$datum['order_no']}开奖失败!", $e->getMessage(), 0);
            }
        }
    }
    ...
    public function getDrawList($where, $order = null)
    {
        ...
        $page = self::where($where)
            ->alias('jo')
            ->leftJoin('jcdc_base jb', 'jo.match_num=jb.match_num')
            ->field([
                'jo.id' => 'openid', // 开奖表ID
                'jo.status', // 开奖状态
                'jo.half_score', // 半场比分
