 ...
 * @blog https://cnblogs.com/hellow-world
 */
class JclqOpen extends BaseModel
{
    use Data;
    ...
    public function getDrawList($where, $order = null)
    {
        ...
        $page = self::where($where)
            ->alias('jo')
            ->leftJoin('jclq_base jb', 'jo.match_num=jb.match_num')
            ->field([
                'jo.id' => 'openid', // 开奖表ID
                'jo.status', // 开奖状态
                'jo.host_score', // 主队全场得分(不含加时赛)
                'jo.guest_score', // 客队全场得分(不含加时赛)
                'jb.rqs',  // 让球数
                'jb.match_num', // 赛事编号
                'jb.league_name', // 联盟名称
                'jb.host_name', // 主队名称
    ...
    public static function isResult($matchNum)
    {
        ...
        if (!empty($data['host_score']) && !empty($data['guest_score'])) {
            return true;
        }
    ...
    public function getJcResult($matchNum)
    {
        ...
        return $this->computedJcResult(Config::LC_CODE, $data);
    }
    ...
    public function handOpen($matchNum)
    {
        ...
        $openDetailData = [
            ...
            'host_score' => $openDetail['host_score'], // 主队全场得分
            'guest_score' => $openDetail['guest_score'], // 客队全场得分
        ];

        //+---------------------------------------------------------------------------------------
        //| 比赛结果处理为竞彩结果
        //+---------------------------------------------------------------------------------------
        //| 数据格式 : ['sf' => 'W', 'rfsf' => 'D', 'ksfc' => '1-5', 'zsfc' => '6-10', 'dxf' => 'L']
        //+---------------------------------------------------------------------------------------
        $dealData = $this->matchResult2JcResult(Config::LC_CODE, $openDetailData);
        // 获取彩种ID
        $lotteryId = Lottery::getIdByCode(Config::LC_CODE);
        // 所有订单详情数据
        ...
        foreach ($data as $datum) {
            try {
                ...
                Db::startTrans();
                // 处理订单详情中的投注项数据格式
                $betBody = $this->baskOrderDetailBet($datum['play_type'], $datum['bet']);
                ...
                $isComplete = false;
                // 记录系统日志
                Helper::log('系统', '竞彩篮球手动开奖', "注单{$datum['order_no']}开奖失败!", $e->getMessage(), 0);
            }
        }

        // 设置该赛事已停售
        JclqBase::where('match_num', $matchNum)->setField('sale_status', 0);
        // 判断是否完成开奖
        if ($isComplete) {
            return true;
        }
     ...
     * @api *
     */
    public function lcAutoDraw($matchNum)
    {
        // 获取赛事结果
        $openDetail = $this->getMatchDetail($matchNum);
        if (
        ...
        foreach ($data as $datum) {
            try {
                ...
                Db::rollback();
                // 写入日志
                Helper::log('系统', '竞彩篮球自动开奖', "注单{$datum['order_no']}开奖失败!", $e->getMessage(), 0);
            }
        }
    }
    ...
    public function exportData($where, $order = 'jo.id DESC')
    {
        ...
        Helper::exportExcel(
            'MatchResultExcel',
            [
                '赛事编号', '联盟名称', '主队名称', '客队名称', '竞彩日期', '开奖状态',
                '让球数', '主队全场得分(不含加时赛)', '客队全场得分(不含加时赛)', '彩种名称', '彩种代码',
            ],
            $data
        );
    }
