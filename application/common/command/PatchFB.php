 ...
 * @blog https://cnblogs.com/hellow-world
 */
class PatchFB extends BaseCommand
{
    /**
     * 配置指令
     *
    ...
    {
        $this->setName('patchFB')
            ->addUsage('php think patchFB')
            ->setDescription('竞彩足球数据爬取');
    }
    ...
    protected function handler()
    {
        ...
        try {
            Db::startTrans();
            // step: 1 设置当前彩种运行状态，设置为已启用上线。
            PatchModel::setRunStatus(Config::ZC_CODE);
            // step: 2 实时爬取竞彩赛事编号列表和对阵数据(两个彩期)
            $this->patchCurrentJcMatchNumData(Config::ZC_CODE, self::FB_LIST_URI);
            Db::commit();
        } catch (\Exception $e) {
            Db::rollback();
            // 再次记录爬取日志, 捕捉sql异常
            PatchLog::log(Config::ZC_CODE, '实时拉取竞彩足球赛事列表发生错误', $e->getMessage());
            // 终止, 打印异常并再次重试
            return $e->getMessage();
        }
        ...
        try {
            Db::startTrans();
            // step: 3 查看是否存在近期取消/腰斩/延期的赛事
            self::setExceptionMatch();
            Db::commit();
        } catch (\Exception $e) {
            Db::rollback();
            // 再次记录爬取日志, 捕捉sql异常
            PatchLog::log(Config::ZC_CODE, '实时爬取异常赛事发生错误', $e->getMessage());
        }
        ...
        try {
            ...
            Db::rollback();
            // 再次记录爬取日志, 捕捉sql异常
            PatchLog::log(Config::ZC_CODE, '实时写入/更新竞彩赔率发生错误', $e->getMessage());
        }
        ...
        try {
            ...
            Db::commit();
            // step: 6 记录成功日志
            $this->executeSuccess(Config::ZC_CODE);
        } catch (\Exception $e) {
            Db::rollback();
            // 再次记录爬取日志, 捕捉sql异常
            PatchLog::log(Config::ZC_CODE, '实时爬取赛事结果发生错误', $e->getMessage());
            return $e->getMessage();
        }
    ...
    {
        foreach (self::$_jcTime as $jcDate) {
            // 获取奖金指数数据
            $api = self::API_HOST . self::FB_RT_URI . $jcDate;
            ...
            if (isset($dealData['LotteryS']) && !empty($dealData['LotteryS'])) {
                ...
                foreach ($data as $item) {
                    ...
                    // 获取该赛事的奖金指数数据
                    $matchIndexData = JczqMatch::getFieldsByWhere(['match_num' => $matchNum], [
                        'sp_spf', // 胜平负
                        'sp_rqspf', // 让球胜平负
                        'sp_bf', // 比分
                        'sp_jqs', // 进球数
                    ...
                    if (empty($matchIndexData)) {
                        ...
                        // 获取奖金指数变化数据, 有可能没有数据
                        $varApi = self::API_HOST . self::FB_VAR_URI . $matchNum;
                        ...
                        // 模拟乐观锁机制,再次检查
                        $repeatCheckId = JczqMatch::getValByWhere(['match_num' => $matchNum], 'id');
                        if ($repeatCheckId) {
                            continue;
                        }

                        // 写入数据
                        $result = JczqMatch::quickCreate($insertData);
                        if (!$result) {
                            PatchLog::log(Config::ZC_CODE, '初次写入竞彩奖金指数数据集失败', var_export($insertData, true));
                            trigger_error('Active Get Data Fails, Class `PatchFB` Lines 230', E_USER_ERROR);
                        }
                    ...
                    if (
                        ...
                        JczqBase::where('match_num', $matchNum)->setField('rqs', $item['HHDA']['H']);
                    }
                    ...
                    // 更新数据
                    JczqMatch::where('match_num', $matchNum)->update($updateData);
                }
            }
        }
    }
     ...
     * @api *
     */
    public static function getZcDetail($matchId, $jcCode, $isAh)
    {
        ...
        if (!isset($responseData['Competition'])) {
            ...
            trigger_error('Patch Match Detail Fail, Class `PatchFB` Lines 303', E_USER_ERROR);
        }

        if (empty($responseData['Competition'])) {
            PatchLog::log($jcCode, '爬取赛事详情数据成功, 但数据为空, 系统将再次发起爬取程序!', var_export($responseData, true));
            trigger_error('Patch Match Detail Is Empty, Class `PatchFB` Lines 284', E_USER_WARNING);
        }
    ...
    {
        // 获取未开奖的赛事开赛日期
        $unDrawData = JczqOpen::where('status', 0)
            ->field('match_date')
            ->group('match_date')
            ->select()
            ->toArray();
    ...
    public static function setExceptionMatch()
    {
        ...
        if (isset($result['Games'])) {
            ...
            foreach ($data as $item) {
                ...
                JczqBase::where('match_num', $matchNum)->setField('match_status', $status);
            }
        }
    }
     ...
     * @api *
     */
    public static function setJczqOpenAndDraw($item)
    {
        $matchNum = $item['Id'][0];
        $openData = JczqOpen::getFieldsByWhere(['match_num' => $matchNum], 'status');
        // 存在该赛事
        if (!empty($openData)) {
            $isAh = JczqBase::getValByWhere(['match_num' => $matchNum], 'isAh');
            ...
            if ($openStatus === 0) {
                ...
                $updateData = [
                    'half_score' => $halfScore, // 半场比分
                    'total_score' => $totalScore, // 含加时总比分
                    'normal_score' => $normalScore, // 不含加时总比分
                    'kick_score' => $kickScore, // 点球比分
                    'status' => $status,
                    'update_at' => Helper::timeFormat(time(), 's'),
                ];
                // 更新赛事结果
                JczqOpen::where('match_num', $matchNum)->setField($updateData);
                // 赛事结束后, 系统自动开奖
                if ($status === 1) {
                    // ... 自动开奖逻辑
                    Order::autoDraw(Config::ZC_CODE, $matchNum);
                }
            }
        }
    }
