...
~function ($, angular) {
    ...
    angular.module("myApp").controller("betExtendCtrl", ["$scope", "$http", "$tip","$location" ,function ($scope, $http, $tip,$location) {
        ...
        $scope.requestApi = function (url, param) {
            url = url || "/vp/bet/pushOrder"; // 推单列表接口
            param = param || {};
            $scope.url = url;
            $scope.param = param;
        ...
        $scope.export = function () {
            ...
            // 导出
            window.location.href = '/vp/bet/exportPush?' + query;
        };
        ...
        $scope.batchAudit = function (state) {
            ...
            // 发送审核请求
            $http.get('/vp/bet/audit?id=' + ids + '&state=' + state).success(function (result) {
                if (result.code === 1) {
        ...
        $scope.detail = function (id) {
            // 发送ajax
            $http.get("/vp/bet/flowList?id=" + id).success(function (result) {
        ...
        $scope.pushDetail = function (id, code) {
            ...
            $http.get("/vp/bet/pushInfo?id=" + id).success(function (result) {
