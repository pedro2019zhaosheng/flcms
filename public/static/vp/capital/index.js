...
~function ($, angular, window) {
    ...
    angular.module("myApp").controller("cashCtrl", ["$scope", "$http", "$tip", "$location", function ($scope, $http, $tip, $location, $compile) {
        ...
        $scope.requestApi = function (url, param) {
            url = url || "/vp/capital/index?model=FundWithdraw";
            param = param || {};
            $scope.url = url;
            $scope.param = param;
        ...
        $scope.export = function () {
            ...
            // 导出
            window.location.href = '/vp/capital/export?model=FundWithdraw&' + query;
        };
        ...
        $scope.submitCash = function () {
            ...
            $.post("/vp/capital/editVerify", $.param({
                id: $('#verifyID').val(),
                model: 'FundWithdraw',
                status: status,
        ...
        $scope.getDetail = function (memberId, fundId) {
            $http.get("/vp/capital/info?memberId=" + memberId + "&model=FundWithdraw&fundId=" + fundId).success(function (result) {
                if (result.code === 1) {
