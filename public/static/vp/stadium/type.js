...
~function ($, angular) {
    ...
    angular.module("myApp").controller("newsTypeCtrl", ["$scope", "$http", "$tip", function ($scope, $http, $tip) {
        ...
        $scope.requestApi = function (url, param) {
            url = url || "/vp/stadium/index?type=1";
            param = param || {};
            $scope.url = url;
            $scope.param = param;
        ...
        $scope.deleteType = function (adverId) {
            $.swal("确定要删除么", function () {
                $http.get("/vp/stadium/deleteType?id=" + adverId).success(function (result) {
                    if (result.code === 1) {
                        $.swalSuccess();
                        // 刷新接口
        ...
        $scope.updateToggle = function (adverId) {
            $http.get("/vp/stadium/info?id=" + adverId + '&type=1').success(function (result) {
                if (result.code === 1) {
