...
~function ($, angular) {
    "use strict";
    angular.module("myApp").controller("adminDetail", ["$scope", "$http", "$tip", function ($scope, $http, $tip) {
        $.busyLoadFull('show', {animation: "fade"});
        // 请求接口
        $scope.requestApi = function (url) {
            url = url || '/vp/admin/adminDetail';
        ...
        $scope.updateAdminDetail = function () {
            ...
            if (email.val() && !myregEmail.test(email.val())) {
                $tip.warning("邮箱格式不正确");
                return;
            }
            ...
            if (!/^1[3-9][0-9]\d{8}$/u.test(phone.val())) {
                $tip.warning("手机号格式错误");
                return;
            }
            ...
            $http.post("/vp/admin/adminModify", $.param(dataObj)).success(function (result) {
                if (result.code === 1) {
