    ...
    angular.module("myApp").controller("newsCtrl", ["$scope", "$http", "$tip", function ($scope, $http, $tip) {
        ...
        $scope.requestApi = function (url, param) {
            ...
            $http.get(url, param).success(function (result) {
                ...
                $scope.newsPage = result.data.page;
                $scope.newsCurrentPage = result.data.list.current_page;
                $scope.newsTotalPage = result.data.list.total;
                $scope.newsPerPage = result.data.list.per_page;
        ...
        $scope.getRoles = function () {
            $http.get("/vp/news/getTypeAll").success(function (result) {
        ...
        $scope.deleteNews = function (newsId) {
            $.swal("确定要删除么", function () {
                $http.get("/vp/news/delete?id=" + newsId).success(function (result) {
                    $.swalSuccess();
                    // 刷新接口
        ...
        $scope.updateToggle = function (newsId) {
            $http.get("/vp/news/info?id=" + newsId).success(function (result) {
                if (result.code === 1) {
                    $("#roleUpdate").val(result.data.newsType).select2();
