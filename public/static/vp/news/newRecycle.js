        ...
        $scope.deleteNews = function (adverId, status) {
            if (status === 1) {
                $.swal("确定要还原么", function () {
                    $http.get("/vp/news/delete?id=" + adverId + '&status=1').success(function (result) {
                        $.swalSuccess();
                        // 刷新接口
                ...
                $.swal("确定要删除么", function () {
                    $http.get("/vp/news/delete?id=" + adverId + '&is_del=1').success(function (result) {
                        $.swalSuccess();
                        // 刷新接口
                        $scope.requestApi($scope.url, $scope.param);
                    });
