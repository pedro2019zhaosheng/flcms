...
~function ($, angular) {
    ...
    angular.module("myApp").controller("agentReturnCtrl", ["$scope", "$http", "$tip", function ($scope, $http, $tip) {
        ...
        $scope.requestApi = function (url, param) {
            url = url || "/vp/Agent/AgentReturnIndex";
            param = param || {};
            $scope.url = url;
            $scope.param = param;
        ...
        $scope.getAgentRebates = function (url) {
            url = url || '/vp/Agent/getAgentRebates';
            $http.get(url).success(function (result) {
        ...
        $scope.getaddAgentRebates = function () {
            $http.get("/vp/agent/getaddAgentRebates").success(function (result) {
        ...
        $scope.addAgentReturn = function (lottery) {
            ...
            // url = url||'/vp/Agent/setAgentrebate';
            $http.post('/vp/Agent/setAgentrebate',
                $.param({
                    Lottery: lotteryData,
                    id: $("#choiceAgent").val(),
                })
        ...
        $scope.deleteAgent = function (agentId) {
            $.swal("确定要删除么", function () {
                $http.get("/vp/Agent/agentRetrunDel?id=" + agentId).success(function (result) {
                    if (result.code) {
                        $.swalSuccess();
                    }
                    // 刷新接口
        ...
        $scope.toggle = function (agentId, status) {
            $http.get("/vp/Agent/agentRetrunStatus?id=" + agentId + "&status=" + status).success(function (result) {
                if (result.code === 1) {
                    if (status === 0) {
                        $tip.success("禁用成功");
                    } else {
