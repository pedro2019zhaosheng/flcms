...
~function ($, angular) {
    ...
    angular.module("myApp").controller("memberCtrl", ["$scope", "$http", "$tip", "$location", function ($scope, $http, $tip, $location) {
        ...
        //下级会员人数
        $scope.lowerMember = function (id) {
            $scope.requestApi('', {params: {topId: id}});
        }
        ...
        $scope.reviseBalance = function (url, memberId) {
            url = url || '/pxy/member/reviseBalance';
            $http.post(url,
                $.param({
                    id: memberId,
        ...
        $scope.getMemberinfo = function (id) {
            $http.get("/pxy/member/getMemberinfo?memberid=" + id).success(function (result) {
                if (result.code === 1) {
        ...
        $scope.updataMember = function (id) {
            $http.post('/pxy/member/updataMember', $.param({
        ...
        $scope.recommend = function (id) {
            window.location.href = '/pxy/recoMember?id=' + id;//pxy/recoMember
        }
        //全选
